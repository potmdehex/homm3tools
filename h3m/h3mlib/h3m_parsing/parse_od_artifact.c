...
int parse_od_artifact(struct H3MLIB_CTX *ctx,
    struct H3M_OD_ENTRY *od_entry, struct META_OD_ENTRY *meta_od_entry)
{
    ...
    struct H3M_OD_BODY_DYNAMIC_ARTIFACT *body = NULL;
    size_t n = 0;
    int ret = 0;

    if (0 ==
        ((struct H3M_OD_BODY_STATIC_ARTIFACT *)&parsing->raw[parsing->offset])->
        has_guardians) {
        meta_od_entry->binary_compatible = 1;
        meta_od_entry->body_size = sizeof(struct H3M_OD_BODY_STATIC_ARTIFACT);
        return 0;
    }

    od_entry->body = calloc(1, sizeof(*body));
    body = (struct H3M_OD_BODY_DYNAMIC_ARTIFACT *)od_entry->body;
