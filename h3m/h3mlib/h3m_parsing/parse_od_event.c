    ...
    struct H3M_OD_ENTRY *od_entry, struct META_OD_ENTRY *meta_od_entry)
{
    ...
    struct H3M_OD_BODY_DYNAMIC_EVENT *body = NULL;
    size_t n = 0;
    int ret = 0;

    od_entry->body = calloc(1, sizeof(*body));
    body = (struct H3M_OD_BODY_DYNAMIC_EVENT *)od_entry->body;
    ...
    n = sizeof(body->applies_to_players)
        + sizeof(body->applies_to_computer) + sizeof(body->cancel_after_visit)
        + sizeof(body->unknown1);
    SAFE_READ_N(&body->applies_to_players, n, parsing);
